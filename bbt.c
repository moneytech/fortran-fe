...
int c;
  ...
  if (c == 0) g95_internal_error("insert(): Duplicate key found!");
...
static g95_bbt *delete_treap(g95_bbt *old, g95_bbt *t,
			     int (*compare)()) {
int c;
  ...
  if (c < 0) t->left = delete_treap(old, t->left, compare);
  if (c > 0) t->right = delete_treap(old, t->right, compare);
  if (c == 0) t = delete_root(t);
