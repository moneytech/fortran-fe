...
g95_constructor *head, *c, *tail=NULL;     
  ...
  case EXPR_ARRAY:
    ...
    result->shape = g95_copy_shape(e->shape, e->rank);      
...
int di, dr, dd, dl, dc, dz;
  ...
  make_generic("iargc", G95_ISYM_IARGC); 
...
g95_actual_arglist *head, *tail, *next;   
  ...
  for(head=*ap; head; head=next) { 
    ...
    if (head->type == EXPR && head->u.expr == NULL) {      
      head->next = NULL;    
...
int i;   
  ...
  for(; formal; formal=formal->next, actual=actual->next, i++) { 
    if (actual->u.expr == NULL) continue; 
...
int flag; 
  ...
  for(actual=expr->value.function.actual; actual; actual=actual->next)      
    if (actual->u.expr != NULL)        
      flag |= (actual->u.expr->ts.type != BT_INTEGER &&  
	       actual->u.expr->ts.type != BT_CHARACTER);        
...
char *name;         
  ...
  if (g95_has_alt_return(c->ext.actual)) return MATCH_NO;          
  ...
  if (g95_intrinsic_symbol(c->sym, 0)) return MATCH_ERROR;        
...
int rank; 
  ...
  if (g95_compare_types(&expr->ts, ts)) return SUCCESS;    
...
/* Insert a pre-resolved function call to the right function */      
  ...
  *expr = *new;
  ...
  if (g95_is_constant_expr(expr->value.function.actual->u.expr) &&     
