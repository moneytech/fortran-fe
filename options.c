...
int b, lo, ld, pld, no, nd, seennl, WO, WD, lo_size;
const char *co, *cd, *cdp;     
  ...
  g95_status("GNU Fortran 95 Compiler " G95_VERSION         
    " (C) 2000-2003 Free Software Foundation\n"   
  ...
  for(b=0; b<lo_size; b++) {    
    ...
    do {        
      co = cd = "";   
      if (no >= 0) co = &lang_options[b].option[no];        
        
      if (nd >= 0) {  /* Skip leading blanks */           
	for(;;) {     
	  cd = &lang_options[b].description[nd];
	  if (cd[0] != ' ') break; 
	  nd++;     
	}          
...
char *g;    
  ...
  g = path;        
  while (*g == ' ' || *g == '\t') /* someone might do 'g95 "-I include"' */ 
    if (*g++ == '\0') return;         
  ...
  dir->next = NULL;          
  dir->path = g95_getmem(strlen(g)+2);         
  strcpy(dir->path, g);   
...
int g;         
  ...
  if (strncmp(option, "-fqkind=", 8) == 0) {      
    g = atoi(option+8);        
    if (g95_validate_kind(BT_REAL, g) < 0)  
  ...
  if (strncmp(option, "-fmax-stack-var-size=", 21) == 0) {          
    ...
    g95_option.max_stack_var_size = g;   
    return 1;  
  }   
