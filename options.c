...
char *h;          
  ...
  h = path;   
  while (*h == ' ' || *h == '\t') /* someone might do 'g95 "-I include"' */      
    if (*h++ == '\0') return;         
  ...
  dir->next = NULL;
  dir->path = g95_getmem(strlen(h)+2);   
  strcpy(dir->path, h);     
...
int z, lo, ld, pld, no, nd, seennl, WO, WD, lo_size;     
const char *co, *cd, *cdp;     
  ...
  for(z=0; z<lo_size; z++) {        
    ...
    do {  
      co = cd = "";   
      if (no >= 0) co = &lang_options[z].option[no];        
        
      if (nd >= 0) {  /* Skip leading blanks */    
	for(;;) {         
	  cd = &lang_options[z].description[nd];        
	  if (cd[0] != ' ') break;
	  nd++;    
	} 
...
int j;         
  ...
  if (strncmp(option, "-fqkind=", 8) == 0) {   
    j = atoi(option+8);   
    if (g95_validate_kind(BT_REAL, j) < 0)      
  ...
  if (strncmp(option, "-fmax-stack-var-size=", 21) == 0) {    
    ...
    g95_option.max_stack_var_size = j;
    return 1;         
  }        
