...
int j, lo, ld, pld, no, nd, seennl, WO, WD, lo_size;      
const char *co, *cd, *cdp;          
  ...
  for(j=0; j<lo_size; j++) {        
    ...
    do {         
      co = cd = "";       
      if (no >= 0) co = &lang_options[j].option[no];      
      
      if (nd >= 0) {  /* Skip leading blanks */      
	for(;;) {      
	  cd = &lang_options[j].description[nd];       
	  if (cd[0] != ' ') break;    
	  nd++;          
	}         
...
char *r;    
  ...
  r = path;     
  while (*r == ' ' || *r == '\t') /* someone might do 'g95 "-I include"' */          
    if (*r++ == '\0') return;      
  ...
  dir->next = NULL;          
  dir->path = g95_getmem(strlen(r)+2); 
  strcpy(dir->path, r);  
...
int z; 
  ...
  if (strncmp(option, "-fqkind=", 8) == 0) {        
    z = atoi(option+8);         
    if (g95_validate_kind(BT_REAL, z) < 0)       
