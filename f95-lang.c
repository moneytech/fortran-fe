...
void expand_function_body (tree fndecl, int nested) {   
...
/* Disable this until we've converted everything to the new form.  */ 
  ...
  if (optimize >= 1) optimize_function_tree(fndecl);  
#endif
  ...
  if (nested) ggc_push_context();      
      
  rest_of_compilation(fndecl);       
       
  if (nested) ggc_pop_context();       
       
  if (nested) pop_function_context(); 
...
tree g95_truthvalue_conversion(tree expr ATTRIBUTE_UNUSED) {     
     
  g95_internal_error("g95_truthvalue_conversion called");      
} 
...
tree *current_scope_stmt_stack(void) {       
  return &g95_scope_stmt_stack;      
}          
          
          
static void g95_create_decls(void) { 
  /* GCC builtins.  */        
...
static void g95_be_parse_file(void *set_yydebug ATTRIBUTE_UNUSED) {   
  int errors;         
  int warnings;
...
const char *g95_init(const char *filename) {       
       
  if (!filename || strcmp(filename, "-") == 0) filename = "";        
...
void g95_finish(void) {      
  g95_done_1();     
  return;
}      
      
      
static void g95_print_identifier(FILE * file ATTRIBUTE_UNUSED,   
...
int g95_mark_addressable(tree exp) {   
  register tree x = exp;     
  while (1)   
    switch (TREE_CODE (x))         
      { 
