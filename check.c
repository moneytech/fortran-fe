...
static void must_be(g95_expr *i, int b, char *thing) {   
   
  g95_error("'%s' argument of '%s' intrinsic at %L must be %s",         
	    g95_current_intrinsic_arg[b], g95_current_intrinsic, &i->where,       
	    thing);
}        
...
static try logical_array_check(g95_expr *array, int j) {       
...
static try int_or_real_check(g95_expr *a, int u) {   
   
  if (a->ts.type != BT_INTEGER && a->ts.type != BT_REAL) {   
    must_be(a, u, "INTEGER or REAL");
    return FAILURE;    
  }    
...
static try array_check(g95_expr *o, int n) {          
          
  if (o->rank != 0) return SUCCESS;    
    
  must_be(o, n, "an array");          
...
static try nonoptional_check(g95_expr *k, int s) {          
          
  if (k->type == EXPR_VARIABLE && k->symbol->attr.optional) {       
    g95_error("'%s' argument of '%s' intrinsic at %L must not be OPTIONAL",         
	      g95_current_intrinsic_arg[s], g95_current_intrinsic, &k->where);     
...
int rank;   
  ...
  if (dim->type != EXPR_CONSTANT ||
      array->type != EXPR_VARIABLE) return SUCCESS;
...
static try type_check(g95_expr *e, int r, bt type) {       
       
  if (e->ts.type == type) return SUCCESS;   
   
  must_be(e, r, g95_basic_typename(type));      
...
static try numeric_check(g95_expr *u, int m) {    
    
  if (g95_numeric_ts(&u->ts)) return SUCCESS;         
         
  must_be(u, m, "a numeric type");        
...
static try same_type_check(g95_expr *j, int h, g95_expr *q, int i) {        
char message[100];        
        
  if (g95_compare_types(&j->ts, &q->ts)) return SUCCESS;          
          
  sprintf(message, "the same type and kind as '%s'",  
	  g95_current_intrinsic_arg[h]);   
...
static try kind_check(g95_expr *y, int r, bt type) {       
int kind;        
        
  if (y == NULL) return SUCCESS;          
          
  if (type_check(y, r, BT_INTEGER) == FAILURE) return FAILURE; 
 
  if (y->type != EXPR_CONSTANT) {    
    must_be(y, r, "a constant"); 
    return FAILURE;          
  } 
 
  if (g95_extract_int(y, &kind) != NULL ||    
...
static try variable_check(g95_expr *q, int k) {     
     
  if ((q->type == EXPR_VARIABLE &&        
       q->symbol->attr.flavor != FL_PARAMETER) ||
      (q->type == EXPR_FUNCTION && q->symbol->result == q->symbol))     
    return SUCCESS;        
        
  if (q->type == EXPR_VARIABLE && q->symbol->attr.intent == INTENT_IN) {          
    g95_error("'%s' argument of '%s' intrinsic at %L cannot be INTENT(IN)",  
	      g95_current_intrinsic_arg[k], g95_current_intrinsic, &q->where);   
    return FAILURE;     
  }     
     
  must_be(q, k, "a variable");    
...
try g95_check_btest(g95_expr *q, g95_expr *pos) {        
        
  if (type_check(q, 0, BT_INTEGER) == FAILURE) return FAILURE;    
...
try g95_check_logical(g95_expr *w, g95_expr *kind) {   
   
  if (type_check(w, 0, BT_LOGICAL) == FAILURE) return FAILURE;      
...
try g95_check_dcmplx(g95_expr *k, g95_expr *r) {      
      
  if (numeric_check(k, 0) == FAILURE) return FAILURE;       
       
  if (r != NULL) {    
    if (numeric_check(r, 1) == FAILURE) return FAILURE;    
    
    if (k->ts.type == BT_COMPLEX) {      
      must_be(r, 1, "not be present if 'x' is COMPLEX"); 
      return FAILURE;   
    }     
  }      
...
try g95_check_set_exponent(g95_expr *u, g95_expr *h) {    
    
  if (type_check(u, 0, BT_REAL) == FAILURE) return FAILURE;          
          
  if (type_check(h, 1, BT_INTEGER) == FAILURE) return FAILURE;    
...
try g95_check_a_p(g95_expr *r, g95_expr *o) {      
      
  if (int_or_real_check(r, 0) == FAILURE) return FAILURE; 
 
  if (same_type_check(r, 0, o, 1) == FAILURE) return FAILURE;    
...
try g95_check_iand(g95_expr *l, g95_expr *c) {  
  
  if (type_check(l, 0, BT_INTEGER) == FAILURE ||       
      type_check(c, 1, BT_INTEGER) == FAILURE) return FAILURE;     
     
  if (same_type_check(l, 0, c, 1) == FAILURE) return FAILURE;          
...
static try double_check(g95_expr *w, int a) {        
        
  if (type_check(w, a, BT_REAL) == FAILURE) return FAILURE; 
 
  if (w->ts.kind != g95_default_double_kind()) {   
    must_be(w, a, "double precision"); 
    return FAILURE;       
  }      
...
try g95_check_ishft(g95_expr *l, g95_expr *shift) { 
...
try g95_check_scan(g95_expr *m, g95_expr *y, g95_expr *d) {        
        
  if (type_check(m, 0, BT_CHARACTER) == FAILURE) return FAILURE;         
         
  if (type_check(y, 1, BT_CHARACTER) == FAILURE) return FAILURE;        
        
  if (d != NULL && type_check(d, 2, BT_LOGICAL) == FAILURE) return FAILURE;    
    
  if (same_type_check(m, 0, y, 1) == FAILURE) return FAILURE;   
...
try g95_check_ior(g95_expr *m, g95_expr *s) {     
     
  if (type_check(m, 0, BT_INTEGER) == FAILURE ||    
      type_check(s, 1, BT_INTEGER) == FAILURE) return FAILURE;    
    
  if (same_type_check(m, 0, s, 1) == FAILURE) return FAILURE;        
...
try g95_check_a_xkind(g95_expr *z, g95_expr *kind) { 
 
  return check_a_kind(z, kind, BT_REAL); 
}
...
try g95_check_digits(g95_expr *o) {         
         
  if (int_or_real_check(o, 0) == FAILURE) return FAILURE;    
...
static try scalar_check(g95_expr *t, int j) {      
      
  if (t->rank == 0) return SUCCESS;     
     
  must_be(t, j, "a scalar");
...
try g95_check_sign(g95_expr *v, g95_expr *b) { 
 
  if (int_or_real_check(v, 0) == FAILURE) return FAILURE;          
          
  if (same_type_check(v, 0, b, 1) == FAILURE) return FAILURE;          
...
try g95_check_scale(g95_expr *a, g95_expr *h) {      
...
try g95_check_char(g95_expr *q, g95_expr *kind) {       
...
try g95_check_int(g95_expr *a, g95_expr *kind) { 
...
try g95_check_idnint(g95_expr *j) {    
    
  if (double_check(j, 0) == FAILURE) return FAILURE;          
...
static try dim_check(g95_expr *dim, int k, int optional) {

  if (optional) {     
    if (dim == NULL) return SUCCESS; 
 
    if (nonoptional_check(dim, k) == FAILURE) return FAILURE;
  ...
  if (type_check(dim, k, BT_INTEGER) == FAILURE) return FAILURE;    
    
  if (scalar_check(dim, k) == FAILURE) return FAILURE;
...
try g95_check_selected_real_kind(g95_expr *p, g95_expr *i) {     
  ...
  if (i != NULL && type_check(i, 1, BT_INTEGER) == FAILURE) return FAILURE;          
...
try g95_check_verify(g95_expr *q, g95_expr *e, g95_expr *n) {        
        
  if (type_check(q, 0, BT_CHARACTER) == FAILURE) return FAILURE;   
   
  if (same_type_check(q, 0, e, 1) == FAILURE) return FAILURE;        
        
  if (n != NULL && type_check(n, 2, BT_LOGICAL) == FAILURE) return FAILURE;  
...
try g95_check_a_ikind(g95_expr *e, g95_expr *kind) {   
   
  return check_a_kind(e, kind, BT_INTEGER); 
}      
      
      
      
      
try g95_check_cmplx(g95_expr *e, g95_expr *t, g95_expr *kind) {          
          
  if (numeric_check(e, 0) == FAILURE) return FAILURE;      
      
  if (t != NULL) {        
    if (numeric_check(t, 1) == FAILURE) return FAILURE;          
          
    if (e->ts.type == BT_COMPLEX) {     
      must_be(t, 1, "not be present if 'x' is COMPLEX");  
      return FAILURE;          
    }
  } 
...
static try kind_value_check(g95_expr *t, int s, int r) {      
char message[100];          
          
  if (t->ts.kind == r) return SUCCESS;          
          
  sprintf(message, "of kind %d", r);   
...
try g95_check_radix(g95_expr *y) {       
       
  if (int_or_real_check(y, 0) == FAILURE) return FAILURE;         
...
try g95_check_present(g95_expr *y) {   
   
  if (variable_check(y, 0) == FAILURE) return FAILURE;

  if (!y->symbol->attr.dummy) {        
    must_be(y, 0, "a dummy variable");        
    return FAILURE;       
  }

  if (!y->symbol->attr.optional) {    
    must_be(y, 0, "an OPTIONAL dummy variable");
    return FAILURE;      
  }      
...
try g95_check_ieor(g95_expr *e, g95_expr *s) {          
          
  if (type_check(e, 0, BT_INTEGER) == FAILURE ||    
      type_check(s, 1, BT_INTEGER) == FAILURE) return FAILURE;    
    
  if (same_type_check(e, 0, s, 1) == FAILURE) return FAILURE;          
...
try g95_check_range(g95_expr *o) {

  if (numeric_check(o, 0) == FAILURE) return FAILURE;    
...
try g95_check_kind(g95_expr *u) {          
          
  if (u->ts.type == BT_DERIVED) {      
    must_be(u, 0, "a non-derived type");         
    return FAILURE; 
  }
...
try g95_check_ibclr(g95_expr *b, g95_expr *pos) {       
       
  if (type_check(b,   0, BT_INTEGER) == FAILURE ||          
...
int c; 
  ...
  for(; arg; arg=arg->next, c++) {  
    i = arg->u.expr;      
    if (i->ts.type != type || i->ts.kind != kind) {        
      g95_error("'a%d' argument of '%s' intrinsic at %L must be %s(%d)",     
		c, g95_current_intrinsic, &i->where,       
...
try g95_check_nearest(g95_expr *e, g95_expr *h) {

  if (type_check(e, 0, BT_REAL) == FAILURE) return FAILURE;    
    
  if (type_check(h, 1, BT_REAL) == FAILURE) return FAILURE;  
...
static try rank_check(g95_expr *h, int n, int rank) {         
char message[100];  
  
  if (h->rank == rank) return SUCCESS;        
...
try g95_check_minloc_maxloc(g95_expr *array, g95_expr *a, g95_expr *a3) {       
  ...
  if (a3 != NULL) {        
    ...
    if (a != NULL) {      
      if (scalar_check(a, 1) == FAILURE) return FAILURE;  
      if (type_check(a, 1, BT_INTEGER) == FAILURE) return FAILURE; 
    }
  } else { 
    if (a != NULL) {
      switch(a->ts.type) {        
	...
	if (logical_array_check(a, 2) == FAILURE) return FAILURE;      
	break;         
         
      default:      
	type_check(a, 1, BT_INTEGER);  /* Guaranteed to fail */
	return FAILURE; 
      }       
    }      
  }  
...
try g95_check_ishftc(g95_expr *t, g95_expr *shift, g95_expr *size) {  
  
  if (type_check(t, 0, BT_INTEGER) == FAILURE ||
      type_check(shift, 1, BT_INTEGER) == FAILURE) return FAILURE;      
...
try g95_check_ibits(g95_expr *v, g95_expr *pos, g95_expr *len) {     
     
  if (type_check(v,   0, BT_INTEGER) == FAILURE ||  
      type_check(pos, 1, BT_INTEGER) == FAILURE ||    
      kind_value_check(pos, 1, g95_default_integer_kind()) == FAILURE ||      
...
try g95_check_x_ni(g95_expr *w) {          
          
  if (type_check(w, 0, BT_REAL) == FAILURE) return FAILURE;        
...
try g95_check_precision(g95_expr *j) {         
         
  if (j->ts.type != BT_REAL && j->ts.type != BT_COMPLEX) {       
    must_be(j, 0, "of type REAL or COMPLEX");
    return FAILURE;    
  }    
...
try g95_check_repeat(g95_expr *a, g95_expr *b) {         
         
  if (type_check(a, 0, BT_CHARACTER) == FAILURE) return FAILURE;       
       
  if (type_check(b, 0, BT_INTEGER) == FAILURE) return FAILURE;   
...
try g95_check_real(g95_expr *x, g95_expr *kind) {    
...
try g95_check_ibset(g95_expr *c, g95_expr *pos) {
...
g95_expr *a;  
  ...
  if (a->ts.type != BT_INTEGER && a->ts.type != BT_REAL) {   
    g95_error("'a1' argument of '%s' intrinsic at %L must be INTEGER or REAL",  
	      g95_current_intrinsic, &a->where);      
    return FAILURE;         
  }    
    
  return check_rest(a->ts.type, a->ts.kind, arg);     
}  
...
int x; 
  ...
  x = mpz_cmp_ui(size, G95_MAX_DIMENSIONS);          
  mpz_clear(size);   
